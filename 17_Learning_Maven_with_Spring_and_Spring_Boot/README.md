...
9. [Spring 프로젝트 버전 관리 규칙 알아보기](#9단계---spring-프로젝트-버전-관리-규칙-알아보기)
...
#### Spring 프로젝트의 버전 규칙
- '주 버전.부 버전.패치 버전[-수정자]'
  - 주 버전 : 주요한 기능 변경이나 호환성이 없는 큰 변화를 나타내며. API의 중대한 변경이나 프레임워크의 핵심 기능이 크게 바뀔 때 증가
  - 부 버전 : 새로운 기능이 추가되었지만, 기존 기능과의 호환성은 유지되는 경우 증가
    - 주 버전과 부 버전을 나누는 가장 중요한 조건은 '버전 업그레이드를 했을 때 이전 버전의 라이브러리를 사용한 코드에 영향을 주는지' 여부이다.
  - 패치 버전 : 버그 수정이나 보안 패치와 같은 작은 변경사항, 기능 추가는 존재하지 않음
  - 수정자 : 특정 단계의 릴리스를 나타내는 추가 정보
    - SNAPSHOT: 개발 중인 버전
    - M1, M2 등: 마일스톤 릴리스 (중간 단계)
    - RC1, RC2 등: 릴리스 후보(Release Candidate)
    - RELEASE: 최종 릴리스 버전
      - RELEASE는 생략하기도 한다.
